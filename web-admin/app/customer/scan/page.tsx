...
export default function CustomerScanPage() {
  ...
  const handlePair = async (code: string) => {
    ...
    try {
      ...
      const res = await CustomerApiService.pairSession(cleanCode, 'CUST_001');
  ...
  return (
    <div className="flex flex-col flex-1 p-4 space-y-4">
      ...
      <div className="relative bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden shadow-2xl flex flex-col items-center justify-center min-h-[300px]">
        ...
        {!scannerStarted && (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center bg-slate-900/90 backdrop-blur-sm z-10">
            ...
            <p className="text-sm font-semibold text-slate-300 mb-1">Đang yêu cầu quyền Camera...</p>
            <p className="text-xs text-slate-500 max-w-xs">
              Vui lòng cho phép truy cập Camera để quét mã QR trên màn hình xe đẩy thông minh.
            </p>
          </div>
        )}
      ...
      {successMsg && (
        <div className="bg-emerald-500/10 border border-emerald-500/40 rounded-2xl p-3 flex items-center gap-2.5 text-xs text-emerald-300">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 space-y-3 shadow-lg">
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 h-12 bg-slate-800/80 border border-slate-700/80 rounded-2xl px-4 text-sm font-bold text-slate-100 uppercase tracking-wider focus:outline-none focus:border-emerald-400 transition-colors"
          />
          <button
            onClick={() => handlePair(manualCode)}
            disabled={isPairing || !manualCode.trim()}
            className="h-12 px-5 rounded-2xl bg-emerald-500 hover:bg-emerald-400 active:scale-95 disabled:opacity-50 text-slate-950 font-bold text-xs uppercase tracking-wider transition-all"
          >
            {isPairing ? 'Đang ghép...' : 'Ghép Xe'}
          </button>
        </div>
        <p className="text-[11px] text-slate-500">
          Mã xe được in ở góc màn hình máy tính bảng trên tay cầm xe đẩy.
        </p>
      </div>
    </div>
  );
